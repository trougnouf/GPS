          ...
          if (gps_alm[i].inc > 0.0)
            {
              ...
              fprintf (out, "Eccentricity:               %+012.9e\n", gps_alm[i].ety);
              fprintf (out, "Time of Applicability(s):   %+012.9e\n", gps_alm[i].toa);
              fprintf (out, "Orbital Inclination(rad):   %+012.9e\n", gps_alm[i].inc);
              fprintf (out, "Rate of Right Ascen(R/s):   %+012.9e\n", gps_alm[i].rra);
              fprintf (out, "SQRT(A) (m^1/2):            %+012.9e\n", gps_alm[i].sqa);
              fprintf (out, "Right Ascen at TOA(rad):    %+012.9e\n", gps_alm[i].lan);
              fprintf (out, "Argument of Perigee(rad):   %+012.9e\n", gps_alm[i].aop);
              fprintf (out, "Mean Anom(rad):             %+012.9e\n", gps_alm[i].ma);
              fprintf (out, "Af0(s):                     %+012.9e\n", gps_alm[i].af0);
              fprintf (out, "Af1(s/s):                   %+012.9e\n", gps_alm[i].af1);
              fprintf (out, "week:                       %04d\n",     gps_alm[i].week % 1024);
              fprintf (out, "\n");
            }
        }
      fclose (out);
      ...
      if (gps_eph[i].inc0 > 0.0)
        {
          ...
          fprintf (out, "Week:                       %04d\n",      gps_eph[i].week);
          fprintf (out, "URA:                        %03d\n",      gps_eph[i].ura);
          fprintf (out, "E Time of Applic(s):        %+014.11e\n", gps_eph[i].toe);
          fprintf (out, "IODE:                       %04d\n",      gps_eph[i].iode);
          fprintf (out, "C Time of Applic(s):        %+014.11e\n", gps_eph[i].toc);
          fprintf (out, "IODC:                       %04d\n",      gps_eph[i].iodc);
          fprintf (out, "Tgd(s):                     %+014.11e\n", gps_eph[i].tgd);
          fprintf (out, "Af0(s):                     %+014.11e\n", gps_eph[i].af0);
          fprintf (out, "Af1(s/s):                   %+014.11e\n", gps_eph[i].af1);
          fprintf (out, "Af2(s/s/s):                 %+014.11e\n", gps_eph[i].af2);
          fprintf (out, "Eccentricity:               %+014.11e\n", gps_eph[i].ety);
          fprintf (out, "Orbital Inclination(rad):   %+014.11e\n", gps_eph[i].inc0);
          fprintf (out, "inc rate (r/s)              %+014.11e\n", gps_eph[i].idot);
          fprintf (out, "Rate of Right Ascen(R/s):   %+014.11e\n", gps_eph[i].omegadot);
          fprintf (out, "SQRT(A) (m^1/2):            %+014.11e\n", gps_eph[i].sqra);
          fprintf (out, "dn                          %+014.11e\n", gps_eph[i].dn);
          fprintf (out, "Right Ascen at TOE(rad):    %+014.11e\n", gps_eph[i].w0);
          fprintf (out, "Argument of Perigee(rad):   %+014.11e\n", gps_eph[i].w);
          fprintf (out, "Mean Anom(rad):             %+014.11e\n", gps_eph[i].ma);
          fprintf (out, "Cuc(rad):                   %+014.11e\n", gps_eph[i].cuc);
          fprintf (out, "Cus(rad):                   %+014.11e\n", gps_eph[i].cus);
          fprintf (out, "Crc(m):                     %+014.11e\n", gps_eph[i].crc);
          fprintf (out, "Crs(m):                     %+014.11e\n", gps_eph[i].crs);
          fprintf (out, "Cic(rad):                   %+014.11e\n", gps_eph[i].cic);
          fprintf (out, "Cis(rad):                   %+014.11e\n", gps_eph[i].cis);
          fprintf (out, "\n");
        }
    }
  fclose (out);
